...
model = PulsarLightcurveExtraction.varying_background_fourier_model(si[sel], segment_starts[1:nseg], segment_ends[1:nseg], cm[sel,:], sm[sel,:], lc_cos_matrix, lc_sin_matrix) # varying_background_spectral_fourier_model(pi_indices[sel], si[sel], segment_starts[1:nseg], segment_ends[1:nseg], cm[sel,:], sm[sel,:], lc_cos_matrix, lc_sin_matrix)

# Start sampling at a point where the model won't have negative intensity
trace = sample(model, NUTS(Nmcmc, 0.85; adtype=AutoReverseDiff()), MCMCDistributed(), Nmcmc, Nchain)
